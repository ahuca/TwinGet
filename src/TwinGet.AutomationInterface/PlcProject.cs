...
namespace TwinGet.AutomationInterface
{
    public class PlcProject : ITcPlcIECProject3
    {
        ...
        private readonly ProjectFilesDeserialization.PlcProject _plcProjectFile;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        {
            ...
            XmlSerializer serializer = new(typeof(ProjectFilesDeserialization.PlcProject));

            using (StringReader reader = new(xmlContent))
            {
                ProjectFilesDeserialization.PlcProject plcProjectFile = serializer.Deserialize(reader) as ProjectFilesDeserialization.PlcProject ?? throw new InvalidProjectFileFormat("Could not deserialize the provided PLC project file.", path);
